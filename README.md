...
- 投诉处置：热线登记异味投诉并限时关联供应批次、检测证书与配送节点；证据引用即冻结快照（JSON + SHA256），证书或温度记录被撤销时原始快照保留并标记复查，已结案件自动转入复查；支持分派、限时处置、补充材料、结案复核、重复投诉合并不失独立联系人与时间线，查询返回当前责任人、剩余时限、证据完整度与结案理由。
...
app/
  ...
  food/             农产品、检测、运输和风险处置服务
    complaints/     投诉登记、证据快照、分派、限时处置、补充材料、结案复核与合并
  schemas/         管理接口输入模型
